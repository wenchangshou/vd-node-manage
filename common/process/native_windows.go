...
func GetThreadName(id uint32) string {
	tree, _ := SnapshotSysProcesses()
	for _, ps := range tree {
		if ps.ProcessID == int(id) {
...
func KillPPT() {
	tree, _ := SnapshotSysProcesses()
	for _, ps := range tree {
		if strings.Compare(ps.ProcessName, "POWERPNT.EXE") == 0 || strings.Compare(ps.ProcessName, "ZOOLON_PPTPlayer.exe") == 0 {
...
func GetUe4ProcessId(processID uint32) (int, error) {
	...
	if len(processArr) == 0 {
		return -1, errors.New("没有找到对应进程")
	}
	for _, p := range processArr {
		processInfo, err := ps.OpenProcess(p)
		if err == nil {
			status, err := processInfo.Stats()
			if err == nil {
				if status.Memory.WorkingSetSize > max {
					max = status.Memory.WorkingSetSize
					activeId = p
				}
			}
		}
...
func KillVideoPlayer() {
	tree, _ := SnapshotSysProcesses()
	for _, ps := range tree {
		if strings.Compare(ps.ProcessName, "ZOOLON_VideoPlayer.exe") == 0 {
			KillProcesses([]int{ps.ProcessID})
		}
	}
}
...
func GetProcessIdByName(name string) (int, error) {
	tree, _ := SnapshotSysProcesses()
	for _, ps := range tree {
		if strings.Compare(ps.ProcessName, name) == 0 {
			return ps.ProcessID, nil
		}
	}
	return 0, errors.New("没有找到对应的线程")
}
